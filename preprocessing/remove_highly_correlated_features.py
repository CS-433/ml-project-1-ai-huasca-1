...
def remove_highly_correlated_features(X, threshold=0.9):
    ...
    correlation_matrix = np.corrcoef(
        X, rowvar=False
    )  # The correlation matrix reveals which features are linearly dependent, helping identify redundant information.
    ...
    correlation_matrix = np.abs(
        correlation_matrix
    )  # take absolute value, so both positive and negative correlation are considered equaly
    ...
    for i in range(correlation_matrix.shape[0]):
        for j in range(i + 1, correlation_matrix.shape[1]):
            ...
            ):  # remove if correlation is above threshold
